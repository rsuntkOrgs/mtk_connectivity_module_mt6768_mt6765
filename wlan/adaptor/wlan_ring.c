 ...
 */
unsigned int adaptor_wlan_ring_read_prepare(unsigned int sz,
 ...
 */
unsigned int adaptor_wlan_ring_write_prepare(unsigned int sz,
						struct ring_segment *seg,
						struct ring *ring)
{
	unsigned int wt = ring->write;
...
unsigned int adaptor_wlan_ring_overwrite_prepare(unsigned int sz, struct ring_segment *seg,
						      struct ring *ring)
{
	unsigned int wt = ring->write;
	unsigned int rd = ring->read;
...
void __adaptor_wlan_ring_segment_prepare(unsigned int from, unsigned int sz,
						struct ring_segment *seg,
						struct ring *ring)
{
...
void _adaptor_wlan_ring_segment_prepare(unsigned int from,
					struct ring_segment *seg,
					struct ring *ring)
{
	__adaptor_wlan_ring_segment_prepare(from, seg->remain, seg, ring);
}

void _adaptor_wlan_ring_segment_prepare_item(unsigned int from,
					struct ring_segment *seg,
					struct ring *ring)
{
	unsigned int size;
